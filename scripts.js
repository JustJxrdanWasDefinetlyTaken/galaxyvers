...
// ===== CONSOLE INTEGRATION =====
console.log('📦 Loading scripts.js with Dual Authentication System...');
...
(function() {
  window.WebsiteKeyTracker = {
    ...
    init: function() {
      ...
      console.log('🔍 WebsiteKeyTracker initialized (v6.0 - Dual Auth)');
    },
    
    trackKeyUsage: async function(key, website, fingerprintId, wepKey) {
      try {
        ...
        await trackingRef.set({
          ...
          wepKeyPreview: wepKey ? wepKey.substring(0, 8) + '...' : 'N/A',
          timestamp: Date.now(),
        ...
        console.log('✅ Key usage tracked with dual auth:', key, 'on', website);
      } catch (error) {
...
// ===== FIREBASE CROSS-DOMAIN KEY SYSTEM WITH DUAL AUTHENTICATION =====
(function() {
  console.log('🔑 Initializing Dual Authentication System...');
  console.log('🛡️ Security: Fingerprint ID + 256-bit WEP Key');
  ...
  // ===== ENHANCED SECURITY: BROWSER FINGERPRINT =====
  ...
  function generateUniqueWEPKey() {
    try {
      ...
      // Combine to create 26-character hex string (256-bit WEP Key format)
      const baseHash = Math.abs(hash).toString(16).toUpperCase().padStart(13, '0').slice(0, 13);
      const wepKey = baseHash + additionalRandom;
      
      console.log('🔐 Generated 256-bit WEP Key:', wepKey.substring(0, 8) + '...' + wepKey.substring(wepKey.length - 4));
      return wepKey;
    } catch (error) {
  ...
  // ===== CHECK OR CREATE WEP KEY FOR BROWSER =====
  async function getOrCreateWEPKey(database, browserId) {
    try {
      console.log('🔍 Checking for existing WEP Key...');
      
      // Check if this browser already has a WEP key
      const wepKeysRef = database.ref('wepKeys');
      const snapshot = await wepKeysRef.orderByChild('fingerprintId').equalTo(browserId).once('value');
      
      if (snapshot.exists()) {
        // WEP key already exists for this browser
        const wepData = Object.values(snapshot.val())[0];
        console.log('✅ Existing WEP Key found:', wepData.wepKey.substring(0, 8) + '...');
        ...
        await database.ref('wepKeys/' + newWEPKey).set({
          ...
          cannotBeRegenerated: true,
          permanent: true,
          locked: true
        });
        
        console.log('🆕 New 256-bit WEP Key created and PERMANENTLY locked to this browser');
        console.log('🔒 WEP Key Preview:', newWEPKey.substring(0, 8) + '...');
        return newWEPKey;
      }
    } catch (error) {
  ...
  async function verifyDualAuthentication(database, storedKey, browserId) {
    try {
      console.log('🔐 Starting Dual Authentication...');
      ...
      // ===== STEP 1: VERIFY FINGERPRINT ID =====
      console.log('🔍 Step 1/2: Verifying Fingerprint ID...');
      if (keyData.fingerprintId !== browserId) {
        console.error('🚫 FINGERPRINT ID MISMATCH');
        await database.ref('securityLogs/fingerprintMismatch/' + Date.now()).set({
          key: storedKey,
          expectedId: keyData.fingerprintId,
      ...
      console.log('✅ Fingerprint ID verified');
      
      // ===== STEP 2: GET AND VERIFY WEP KEY =====
      console.log('🔍 Step 2/2: Verifying 256-bit WEP Key...');
      const wepKey = await getOrCreateWEPKey(database, browserId);
      
      if (!wepKey) {
        console.error('🚫 Could not retrieve WEP Key');
        return false;
      }
      ...
      if (!keyData.wepKey) {
        ...
        console.log('🔗 256-bit WEP Key linked to access key');
        console.log('✅ Dual Authentication configured');
        return true;
      }
      
      // ===== STEP 3: VERIFY WEP KEY MATCHES =====
      if (keyData.wepKey !== wepKey) {
        console.error('🚫 WEP KEY MISMATCH - CRITICAL SECURITY VIOLATION');
        await database.ref('securityLogs/wepKeyMismatch/' + Date.now()).set({
          key: storedKey,
          expectedWEP: keyData.wepKey.substring(0, 8) + '...',
          attemptedWEP: wepKey.substring(0, 8) + '...',
          browserId: browserId,
          timestamp: Date.now(),
          date: new Date().toISOString(),
          severity: 'CRITICAL',
      ...
      console.log('✅ 256-bit WEP Key verified');
  ...
  waitForFirebase(async function() {
    ...
    async function initializeKeySystem() {
      ...
      console.log('🔐 Generating Browser Fingerprint ID...');
      const browserId = generateBrowserFingerprintId();
      console.log('🔒 Browser Fingerprint ID:', browserId);
      
      // ===== STEP 1: CHECK FIREBASE FOR EXISTING KEY BY ID =====
      console.log('🔍 Step 1: Searching Firebase for key with this Fingerprint ID...');
      ...
      try {
        ...
        if (snapshot.exists()) {
          ...
          for (const [key, keyData] of Object.entries(allKeys)) {
            if (keyData.fingerprintId === browserId) {
              foundKey = key;
              console.log('✅ Found key for this Fingerprint ID:', key);
              break;
            }
          }
          
          if (foundKey) {
            console.log('🔐 Verifying with Dual Authentication...');
            
            const isAuthenticated = await verifyDualAuthentication(database, foundKey, browserId);
            
            if (!isAuthenticated) {
              console.error('🚫 Dual Authentication Failed');
              localStorage.removeItem('galaxyverse_user_key');
              localStorage.removeItem('galaxyverse_access');
              alert('⚠️ Security Alert: Dual authentication failed.');
              showKeyEntryScreen();
              return;
            }
            
            console.log('🎉 Dual Authentication Success - Auto-login!');
            ...
            if (typeof window.WebsiteKeyTracker !== 'undefined') {
              window.WebsiteKeyTracker.trackKeyUsage(foundKey, actualSite, browserId, keyData.wepKey);
            }
        ...
        console.log('ℹ️ No key found for this Fingerprint ID');
      } catch (error) {
      ...
      if (storedKey) {
        console.log('📦 Found key in localStorage, verifying with Dual Authentication...');
        try {
          // Use dual authentication
          const isAuthenticated = await verifyDualAuthentication(database, storedKey, browserId);
          
          if (!isAuthenticated) {
            console.error('🚫 DUAL AUTHENTICATION FAILED');
            localStorage.removeItem('galaxyverse_user_key');
            localStorage.removeItem('galaxyverse_access');
            alert('⚠️ Security Alert: Dual authentication failed. Please re-enter your key.');
            showKeyEntryScreen();
            return;
          }
          
          console.log('✅ Dual Authentication verified');
          
          // Authentication successful - update access info
          const keyRef = database.ref('usedKeys/' + storedKey);
          const keyData = (await keyRef.once('value')).val();
          const websites = keyData.websites || [];
          ...
          if (typeof window.WebsiteKeyTracker !== 'undefined') {
            window.WebsiteKeyTracker.trackKeyUsage(storedKey, actualSite, browserId, keyData.wepKey);
          }
    ...
    function showKeyEntryScreen() {
      console.log('🔐 Showing key entry screen with Dual Authentication info');
      ...
      keyOverlay.innerHTML = `
        ...
        ">
          ...
          <div style="
            ...
            font-size: 12px;
          ">
            🛡️ Dual Authentication Protection<br>
            🔐 Fingerprint ID + 256-bit WEP Key<br>
            🔒 Keys locked to ONE browser permanently<br>
            🌟 Cannot be transferred or regenerated<br>
      ...
      testConnectionBtn.addEventListener('click', async function() {
        ...
        try {
          ...
          keyError.textContent = '✅ Connection working! Dual Auth ready.';
      ...
      async function verifyKey() {
        ...
        submitBtn.textContent = 'Verifying with Dual Auth...';
        submitBtn.style.cursor = 'wait';
        
        console.log('🔑 Verifying key with Dual Authentication:', enteredKey);
        ...
        try {
          ...
          if (snapshot.exists()) {
            // ===== KEY EXISTS - USE DUAL AUTHENTICATION =====
            const keyData = snapshot.val();
            
            console.log('📝 Key found in database');
            console.log('🔍 Verifying dual authentication...');
            
            const isAuthenticated = await verifyDualAuthentication(database, enteredKey, browserId);
            
            if (!isAuthenticated) {
              keyError.textContent = '🚫 Dual Authentication failed. This key is locked to a different browser.';
              keyError.style.color = '#ff4444';
              keyError.style.display = 'block';
              keyInput.style.borderColor = '#ff4444';
              keyInput.value = '';
            ...
            if (typeof window.WebsiteKeyTracker !== 'undefined') {
              window.WebsiteKeyTracker.trackKeyUsage(enteredKey, actualSite, browserId, keyData.wepKey);
            }
            
            keyError.style.color = '#4ade80';
            keyError.textContent = '✅ Welcome back! Dual Authentication verified';
            keyError.style.display = 'block';
            ...
            // ===== NEW KEY - REGISTER WITH DUAL AUTHENTICATION =====
            console.log('🆕 New key! Registering with Dual Authentication...');
            submitBtn.textContent = 'Generating WEP Key...';
            
            // Generate WEP Key for this browser
            const wepKey = await getOrCreateWEPKey(database, browserId);
            ...
            if (typeof window.WebsiteKeyTracker !== 'undefined') {
              window.WebsiteKeyTracker.trackKeyUsage(enteredKey, actualSite, browserId, wepKey);
            }

            keyError.style.color = '#4ade80';
            keyError.textContent = '✅ Success! Key registered with Dual Authentication. Works on ALL GalaxyVerse domains!';
            keyError.style.display = 'block';
            keyInput.style.borderColor = '#4ade80';
            submitBtn.style.background = 'linear-gradient(135deg, #4ade80, #22c55e)';
            submitBtn.textContent = 'Success!';

            console.log('✅ New key registered with Dual Authentication');
            console.log('🔐 Fingerprint ID + 256-bit WEP Key locked');
...
const themes = {
  original: { 
    bgColor: '#121826', navColor: '#1e2433', accentColor: '#4f90ff', textColor: '#e0e6f1', borderColor: '#38415d', hoverBg: '#2a2f48', btnBg: '#3b466f', btnHoverBg: '#4f90ff',
    sidebarTop: '#0d1018', sidebarMid: '#0f1420', sidebarBottom: '#091229', sidebarText: 'rgba(224, 230, 241, 0.75)', sidebarTextHover: '#ffffff', sidebarHover: 'rgba(79, 144, 255, 0.15)', sidebarHoverGradient: 'rgba(79, 144, 255, 0.15)', sidebarActiveBg: 'linear-gradient(135deg, rgba(79, 144, 255, 0.25) 0%, rgba(63, 81, 181, 0.25) 100%)', sidebarActiveShadow: 'rgba(79, 144, 255, 0.3)', sidebarAccentBorder: 'linear-gradient(180deg, #4f90ff 0%, #3f51b5 100%)', sidebarAccentGlow: 'rgba(79, 144, 255, 0.6)', sidebarIconGlow: 'rgba(79, 144, 255, 0.6)', sidebarIconActiveGlow: 'rgba(79, 144, 255, 0.6)'
  },
  halloween: { 
    bgColor: '#0a0a0a', navColor: '#1a0f0a', accentColor: '#ff6600', textColor: '#ffa500', borderColor: '#331a00', hoverBg: '#2a1a0f', btnBg: '#4a2a1a', btnHoverBg: '#ff6600',
    sidebarTop: '#050503', sidebarMid: '#0a0805', sidebarBottom: '#000000', sidebarText: 'rgba(255, 165, 0, 0.7)', sidebarTextHover: '#ffa500', sidebarHover: 'rgba(255, 102, 0, 0.15)', sidebarHoverGradient: 'rgba(255, 102, 0, 0.15)', sidebarActiveBg: 'linear-gradient(135deg, rgba(255, 102, 0, 0.3) 0%, rgba(204, 85, 0, 0.3) 100%)', sidebarActiveShadow: 'rgba(255, 102, 0, 0.4)', sidebarAccentBorder: 'linear-gradient(180deg, #ff6600 0%, #cc5500 100%)', sidebarAccentGlow: 'rgba(255, 102, 0, 0.6)', sidebarIconGlow: 'rgba(255, 102, 0, 0.6)', sidebarIconActiveGlow: 'rgba(255, 102, 0, 0.6)'
  },
  christmas: { 
    bgColor: '#0a1a0a', navColor: '#1a2e1a', accentColor: '#c41e3a', textColor: '#f0f8ff', borderColor: '#2d5016', hoverBg: '#1f3d1f', btnBg: '#2d5016', btnHoverBg: '#c41e3a',
    sidebarTop: '#051005', sidebarMid: '#0a1a0a', sidebarBottom: '#052d05', sidebarText: 'rgba(240, 248, 255, 0.75)', sidebarTextHover: '#f0f8ff', sidebarHover: 'rgba(196, 30, 58, 0.15)', sidebarHoverGradient: 'rgba(196, 30, 58, 0.15)', sidebarActiveBg: 'linear-gradient(135deg, rgba(196, 30, 58, 0.3) 0%, rgba(45, 80, 22, 0.3) 100%)', sidebarActiveShadow: 'rgba(196, 30, 58, 0.4)', sidebarAccentBorder: 'linear-gradient(180deg, #c41e3a 0%, #2d5016 100%)', sidebarAccentGlow: 'rgba(196, 30, 58, 0.6)', sidebarIconGlow: 'rgba(196, 30, 58, 0.6)', sidebarIconActiveGlow: 'rgba(196, 30, 58, 0.6)'
  },
  dark: { 
    bgColor: '#0a0a0a', navColor: '#1a1a1a', accentColor: '#00ff00', textColor: '#ffffff', borderColor: '#333333', hoverBg: '#2a2a2a', btnBg: '#262626', btnHoverBg: '#00ff00',
    sidebarTop: '#1f1f1f', sidebarMid: '#262626', sidebarBottom: '#1a1a1a', sidebarText: 'rgba(255, 255, 255, 0.7)', sidebarTextHover: '#ffffff', sidebarHover: 'rgba(0, 255, 0, 0.15)', sidebarHoverGradient: 'rgba(0, 255, 0, 0.15)', sidebarActiveBg: 'linear-gradient(135deg, rgba(0, 255, 0, 0.25) 0%, rgba(0, 200, 0, 0.25) 100%)', sidebarActiveShadow: 'rgba(0, 255, 0, 0.3)', sidebarAccentBorder: 'linear-gradient(180deg, #00ff00 0%, #00cc00 100%)', sidebarAccentGlow: 'rgba(0, 255, 0, 0.6)', sidebarIconGlow: 'rgba(0, 255, 0, 0.6)', sidebarIconActiveGlow: 'rgba(0, 255, 0, 0.6)'
  },
  light: { 
    bgColor: '#f5f5f5', navColor: '#ffffff', accentColor: '#2563eb', textColor: '#1f2937', borderColor: '#e5e7eb', hoverBg: '#e5e7eb', btnBg: '#d1d5db', btnHoverBg: '#2563eb',
    sidebarTop: '#fafafa', sidebarMid: '#f0f0f0', sidebarBottom: '#e8e8e8', sidebarText: 'rgba(31, 41, 55, 0.7)', sidebarTextHover: '#1f2937', sidebarHover: 'rgba(37, 99, 235, 0.1)', sidebarHoverGradient: 'rgba(37, 99, 235, 0.1)', sidebarActiveBg: 'linear-gradient(135deg, rgba(37, 99, 235, 0.2) 0%, rgba(29, 78, 216, 0.2) 100%)', sidebarActiveShadow: 'rgba(37, 99, 235, 0.3)', sidebarAccentBorder: 'linear-gradient(180deg, #2563eb 0%, #1d4ed8 100%)', sidebarAccentGlow: 'rgba(37, 99, 235, 0.5)', sidebarIconGlow: 'rgba(37, 99, 235, 0.5)', sidebarIconActiveGlow: 'rgba(37, 99, 235, 0.5)'
  },
  midnight: { 
    bgColor: '#0f0f23', navColor: '#1a1a2e', accentColor: '#9d4edd', textColor: '#e0e0e0', borderColor: '#3c3c5a', hoverBg: '#2a2a4e', btnBg: '#3c3c6f', btnHoverBg: '#9d4edd',
    sidebarTop: '#0a0a18', sidebarMid: '#0c0c1e', sidebarBottom: '#060612', sidebarText: 'rgba(224, 224, 224, 0.75)', sidebarTextHover: '#ffffff', sidebarHover: 'rgba(157, 78, 221, 0.15)', sidebarHoverGradient: 'rgba(157, 78, 221, 0.15)', sidebarActiveBg: 'linear-gradient(135deg, rgba(157, 78, 221, 0.3) 0%, rgba(138, 43, 226, 0.3) 100%)', sidebarActiveShadow: 'rgba(157, 78, 221, 0.4)', sidebarAccentBorder: 'linear-gradient(180deg, #9d4edd 0%, #8a2be2 100%)', sidebarAccentGlow: 'rgba(157, 78, 221, 0.6)', sidebarIconGlow: 'rgba(157, 78, 221, 0.6)', sidebarIconActiveGlow: 'rgba(157, 78, 221, 0.6)'
  },
  ocean: { 
    bgColor: '#001f3f', navColor: '#0a2f4f', accentColor: '#00d4ff', textColor: '#cfe2f3', borderColor: '#1a4f6f', hoverBg: '#1a3f5f', btnBg: '#2a5f7f', btnHoverBg: '#00d4ff',
    sidebarTop: '#001529', sidebarMid: '#00182e', sidebarBottom: '#000d1a', sidebarText: 'rgba(207, 226, 243, 0.75)', sidebarTextHover: '#ffffff', sidebarHover: 'rgba(0, 212, 255, 0.15)', sidebarHoverGradient: 'rgba(0, 212, 255, 0.15)', sidebarActiveBg: 'linear-gradient(135deg, rgba(0, 212, 255, 0.25) 0%, rgba(0, 150, 199, 0.25) 100%)', sidebarActiveShadow: 'rgba(0, 212, 255, 0.3)', sidebarAccentBorder: 'linear-gradient(180deg, #00d4ff 0%, #0096c7 100%)', sidebarAccentGlow: 'rgba(0, 212, 255, 0.6)', sidebarIconGlow: 'rgba(0, 212, 255, 0.6)', sidebarIconActiveGlow: 'rgba(0, 212, 255, 0.6)'
  },
  sunset: { 
    bgColor: '#2d1b2e', navColor: '#3d2b3e', accentColor: '#ff6b9d', textColor: '#fce4ec', borderColor: '#5d4b5e', hoverBg: '#4d3b4e', btnBg: '#6d5b6e', btnHoverBg: '#ff6b9d',
    sidebarTop: '#1f1020', sidebarMid: '#24152a', sidebarBottom: '#180d1f', sidebarText: 'rgba(252, 228, 236, 0.75)', sidebarTextHover: '#ffffff', sidebarHover: 'rgba(255, 107, 157, 0.15)', sidebarHoverGradient: 'rgba(255, 107, 157, 0.15)', sidebarActiveBg: 'linear-gradient(135deg, rgba(255, 107, 157, 0.3) 0%, rgba(219, 39, 119, 0.3) 100%)', sidebarActiveShadow: 'rgba(255, 107, 157, 0.4)', sidebarAccentBorder: 'linear-gradient(180deg, #ff6b9d 0%, #db2777 100%)', sidebarAccentGlow: 'rgba(255, 107, 157, 0.6)', sidebarIconGlow: 'rgba(255, 107, 157, 0.6)', sidebarIconActiveGlow: 'rgba(255, 107, 157, 0.6)'
  },
  forest: { 
    bgColor: '#1a2f1a', navColor: '#2a3f2a', accentColor: '#7cb342', textColor: '#e8f5e9', borderColor: '#3a5f3a', hoverBg: '#3a4f3a', btnBg: '#4a6f4a', btnHoverBg: '#7cb342',
    sidebarTop: '#0f1e0f', sidebarMid: '#142414', sidebarBottom: '#0a150a', sidebarText: 'rgba(232, 245, 233, 0.75)', sidebarTextHover: '#ffffff', sidebarHover: 'rgba(124, 179, 66, 0.15)', sidebarHoverGradient: 'rgba(124, 179, 66, 0.15)', sidebarActiveBg: 'linear-gradient(135deg, rgba(124, 179, 66, 0.3) 0%, rgba(85, 139, 47, 0.3) 100%)', sidebarActiveShadow: 'rgba(124, 179, 66, 0.4)', sidebarAccentBorder: 'linear-gradient(180deg, #7cb342 0%, #558b2f 100%)', sidebarAccentGlow: 'rgba(124, 179, 66, 0.6)', sidebarIconGlow: 'rgba(124, 179, 66, 0.6)', sidebarIconActiveGlow: 'rgba(124, 179, 66, 0.6)'
  },
  purple: { 
    bgColor: '#1a0a2e', navColor: '#2a1a3e', accentColor: '#b744f7', textColor: '#f0e6ff', borderColor: '#4a3a5e', hoverBg: '#3a2a4e', btnBg: '#5a4a6e', btnHoverBg: '#b744f7',
    sidebarTop: '#100520', sidebarMid: '#150a29', sidebarBottom: '#0a0515', sidebarText: 'rgba(240, 230, 255, 0.75)', sidebarTextHover: '#ffffff', sidebarHover: 'rgba(183, 68, 247, 0.15)', sidebarHoverGradient: 'rgba(183, 68, 247, 0.15)', sidebarActiveBg: 'linear-gradient(135deg, rgba(183, 68, 247, 0.3) 0%, rgba(147, 51, 234, 0.3) 100%)', sidebarActiveShadow: 'rgba(183, 68, 247, 0.4)', sidebarAccentBorder: 'linear-gradient(180deg, #b744f7 0%, #9333ea 100%)', sidebarAccentGlow: 'rgba(183, 68, 247, 0.6)', sidebarIconGlow: 'rgba(183, 68, 247, 0.6)', sidebarIconActiveGlow: 'rgba(183, 68, 247, 0.6)'
  },
  cyberpunk: { 
    bgColor: '#0d0221', navColor: '#1a0b3a', accentColor: '#ff006e', textColor: '#00f5ff', borderColor: '#8338ec', hoverBg: '#2a1a4a', btnBg: '#3a2a5a', btnHoverBg: '#ff006e',
    sidebarTop: '#070112', sidebarMid: '#0a0219', sidebarBottom: '#050110', sidebarText: 'rgba(0, 245, 255, 0.75)', sidebarTextHover: '#00f5ff', sidebarHover: 'rgba(255, 0, 110, 0.15)', sidebarHoverGradient: 'rgba(255, 0, 110, 0.15)', sidebarActiveBg: 'linear-gradient(135deg, rgba(255, 0, 110, 0.3) 0%, rgba(131, 56, 236, 0.3) 100%)', sidebarActiveShadow: 'rgba(255, 0, 110, 0.4)', sidebarAccentBorder: 'linear-gradient(180deg, #ff006e 0%, #8338ec 100%)', sidebarAccentGlow: 'rgba(255, 0, 110, 0.6)', sidebarIconGlow: 'rgba(255, 0, 110, 0.6)', sidebarIconActiveGlow: 'rgba(255, 0, 110, 0.6)'
  },
  matrix: { 
    bgColor: '#000000', navColor: '#0a1a0a', accentColor: '#00ff41', textColor: '#00ff41', borderColor: '#003b00', hoverBg: '#0a2a0a', btnBg: '#1a3a1a', btnHoverBg: '#00ff41',
    sidebarTop: '#001500', sidebarMid: '#001a00', sidebarBottom: '#000d00', sidebarText: 'rgba(0, 255, 65, 0.7)', sidebarTextHover: '#00ff41', sidebarHover: 'rgba(0, 255, 65, 0.15)', sidebarHoverGradient: 'rgba(0, 255, 65, 0.15)', sidebarActiveBg: 'linear-gradient(135deg, rgba(0, 255, 65, 0.3) 0%, rgba(0, 200, 50, 0.3) 100%)', sidebarActiveShadow: 'rgba(0, 255, 65, 0.4)', sidebarAccentBorder: 'linear-gradient(180deg, #00ff41 0%, #00c832 100%)', sidebarAccentGlow: 'rgba(0, 255, 65, 0.6)', sidebarIconGlow: 'rgba(0, 255, 65, 0.6)', sidebarIconActiveGlow: 'rgba(0, 255, 65, 0.6)'
  },
  neon: { 
    bgColor: '#1a0033', navColor: '#2a0a4a', accentColor: '#ff00ff', textColor: '#00ffff', borderColor: '#4a1a6a', hoverBg: '#3a1a5a', btnBg: '#5a2a7a', btnHoverBg: '#ff00ff',
    sidebarTop: '#100022', sidebarMid: '#15002e', sidebarBottom: '#0a0015', sidebarText: 'rgba(0, 255, 255, 0.75)', sidebarTextHover: '#00ffff', sidebarHover: 'rgba(255, 0, 255, 0.15)', sidebarHoverGradient: 'rgba(255, 0, 255, 0.15)', sidebarActiveBg: 'linear-gradient(135deg, rgba(255, 0, 255, 0.3) 0%, rgba(200, 0, 200, 0.3) 100%)', sidebarActiveShadow: 'rgba(255, 0, 255, 0.4)', sidebarAccentBorder: 'linear-gradient(180deg, #ff00ff 0%, #c800c8 100%)', sidebarAccentGlow: 'rgba(255, 0, 255, 0.6)', sidebarIconGlow: 'rgba(255, 0, 255, 0.6)', sidebarIconActiveGlow: 'rgba(255, 0, 255, 0.6)'
  },
  fire: { 
    bgColor: '#1a0a00', navColor: '#2a1a0a', accentColor: '#ff4500', textColor: '#ffe4b5', borderColor: '#4a2a1a', hoverBg: '#3a1a0a', btnBg: '#5a3a2a', btnHoverBg: '#ff4500',
    sidebarTop: '#0f0500', sidebarMid: '#140a00', sidebarBottom: '#0a0400', sidebarText: 'rgba(255, 228, 181, 0.75)', sidebarTextHover: '#ffe4b5', sidebarHover: 'rgba(255, 69, 0, 0.15)', sidebarHoverGradient: 'rgba(255, 69, 0, 0.15)', sidebarActiveBg: 'linear-gradient(135deg, rgba(255, 69, 0, 0.3) 0%, rgba(204, 50, 0, 0.3) 100%)', sidebarActiveShadow: 'rgba(255, 69, 0, 0.4)', sidebarAccentBorder: 'linear-gradient(180deg, #ff4500 0%, #cc3200 100%)', sidebarAccentGlow: 'rgba(255, 69, 0, 0.6)', sidebarIconGlow: 'rgba(255, 69, 0, 0.6)', sidebarIconActiveGlow: 'rgba(255, 69, 0, 0.6)'
  },
  ice: { 
    bgColor: '#0a1a2a', navColor: '#1a2a3a', accentColor: '#00bfff', textColor: '#e0f7ff', borderColor: '#2a3a4a', hoverBg: '#1a2a3a', btnBg: '#2a4a5a', btnHoverBg: '#00bfff',
    sidebarTop: '#051018', sidebarMid: '#0a1520', sidebarBottom: '#050d15', sidebarText: 'rgba(224, 247, 255, 0.75)', sidebarTextHover: '#ffffff', sidebarHover: 'rgba(0, 191, 255, 0.15)', sidebarHoverGradient: 'rgba(0, 191, 255, 0.15)', sidebarActiveBg: 'linear-gradient(135deg, rgba(0, 191, 255, 0.25) 0%, rgba(0, 150, 199, 0.25) 100%)', sidebarActiveShadow: 'rgba(0, 191, 255, 0.3)', sidebarAccentBorder: 'linear-gradient(180deg, #00bfff 0%, #0096c7 100%)', sidebarAccentGlow: 'rgba(0, 191, 255, 0.6)', sidebarIconGlow: 'rgba(0, 191, 255, 0.6)', sidebarIconActiveGlow: 'rgba(0, 191, 255, 0.6)'
  },
  retro: { 
    bgColor: '#2b1b17', navColor: '#3d2b27', accentColor: '#ff9966', textColor: '#ffeaa7', borderColor: '#5d4b47', hoverBg: '#4d3b37', btnBg: '#6d5b57', btnHoverBg: '#ff9966',
    sidebarTop: '#1d110d', sidebarMid: '#221612', sidebarBottom: '#160d0a', sidebarText: 'rgba(255, 234, 167, 0.75)', sidebarTextHover: '#ffeaa7', sidebarHover: 'rgba(255, 153, 102, 0.15)', sidebarHoverGradient: 'rgba(255, 153, 102, 0.15)', sidebarActiveBg: 'linear-gradient(135deg, rgba(255, 153, 102, 0.3) 0%, rgba(230, 126, 77, 0.3) 100%)', sidebarActiveShadow: 'rgba(255, 153, 102, 0.4)', sidebarAccentBorder: 'linear-gradient(180deg, #ff9966 0%, #e67e4d 100%)', sidebarAccentGlow: 'rgba(255, 153, 102, 0.6)', sidebarIconGlow: 'rgba(255, 153, 102, 0.6)', sidebarIconActiveGlow: 'rgba(255, 153, 102, 0.6)'
  }
};
...
function initializeApp() {
  try {
    console.log('🚀 Initializing GalaxyVerse with Dual Authentication System...');
    console.log('🛡️ Security Layers: Fingerprint ID + 256-bit WEP Key');
  ...
  console.log('✅ GalaxyVerse initialized with Dual Authentication System');
  console.log('📊 Console active - Press Ctrl+Shift+K to toggle');
  console.log('🛡️ Security: Fingerprint ID + 256-bit WEP Key');
  console.log('🔒 Each key locked to ONE browser permanently');
